...
class DistribAlreadyEnabledError(DistribError):
    def __init__(self, distrib, level):
        self.msg = "The distribution %s is already enabled at level %s" % (
            distrib,
            level,
        )
...
class RestoreError(Error):
    def __init__(self, exception, backup_dir):
        self.msg = "Error during restore: %s, please restore files manually from %s" % (
            str(exception),
            backup_dir,
        )
...
class ProfileAlreadyEnabledError(Error):
    def __init__(self, profile_name, profile_level):
        self.msg = "Profile already enabled: %s, level: %s" % (
            profile_name,
            profile_level,
        )
...
class PluginsNotExistError(Error):
    def __init__(self, *plugins):
        self.msg = (
            "Plugin%s " % (len(plugins) > 1 and "s" or "")
            + ", ".join(plugins)
            + " do%s " % (len(plugins) == 1 and "es" or "")
            + "not exist"
        )
